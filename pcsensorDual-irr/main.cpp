...
int main()
{
  ...
  main2(); //init pcsensors usb stuff
            float insin;
        float outsout;
...
gettemps(insin,outsout);
      ...
      if (NextGraph < now)
      {
        ...
        // if (rand()%2==0) // no parallel movement
            graph[n]->addValue(outsout*8 / graph[n]->getMax());  //this might throw graph off with scale issues
            for (u8 n=3; n<5; n++)
        // if (rand()%2==0) // no parallel movement
                graph[n]->addValue(insin*8 / graph[n]->getMax());


        //   printf("in temperature %.2fF\n",insin);
        // if (rand()%10==0)
        // {
           flag = !flag;
